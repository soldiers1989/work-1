    ...
    public String getIdTwo_photo(HttpServletRequest request) {
        ...
        try {
            logger.info("正在执行IdTwophotoServ。。。。。");
            ...
            // 记录客户请求流水
            int re = transDtlService.insertTransDtl("IdTwophotoServ", paramData, raid, "1", merId);
            if (re != 1) {
                logger.error("身份证验证及照片查询，插入请求流水失败！ ");
            }
            // 记录请求数源流水
            int ret = transDtlService.insertTransDtl("brIdTwophotoServ", paramData, raid, "2", merId);
            if (ret != 1) {
            ...
            Map<String, String> map_code = transDtlService.getMarkingCode(request, jsonObject, "IdTwophotoServ");
            ...
            if (type != 2) {
                portrait_result = httpsUtil.postMsg(postBRUrl + "brIdTwophotoServ", "paramData=" + jsonObject.toString());
                ...
                // 取flag
                flag = map.get("flag_IdTwophotoServ".toUpperCase());
                // 更新流水收费标志
                if (String.valueOf(map.get("FLAG_IDTWOPHOTOSERV")).equals("1")) {
                    map.put("IS_FEE", "1");
                    map.put("TID", raid);
                } else {
                    map.put("IS_FEE", "0");
                ...
                if (map.get("code".toUpperCase()).trim().equals("600000")) {// 返回成功有数据的情况下，才会插入所有的表
                    ...
                    // 主表插入
                    IdTwophotoServService.insert(map);
                    if (type == 0) {
                        IdTwophotoServService.insertS(map);
                    } else {
                        IdTwophotoServService.updateS(map);// 数据库入表实时表更新
                    }
                } else {
                    // 接口调用异常
            ...
            List<Map<String, String>> listM = baseService.selectByStr("orm.mapper.zz.br.IdTwophotoServMapper.selectByCode", marking_cod);
            ...
            logger.error("身份证验证及照片查询请求异常 输出--->:" + e);
        }
        return portrait_result + backTime;
    }