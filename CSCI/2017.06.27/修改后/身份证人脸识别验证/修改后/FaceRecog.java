    ...
    public String FaceRecog(HttpServletRequest request) {
        ...
        try {
            ...
            Map<String, String> map_code = transDtlService.getMarkingCode(request, jsonObject, "FaceRecog");
            ...
            if (type != 2) {
                portrait_result = httpsUtil.postMsg(postBRUrl + "brFaceRecog", "paramData=" + jsonObject.toString());
                ...
                map.put("ZZID", raid);
                // 取flag
                flag = map.get("flag_FaceRecog".toUpperCase());
                // 更新流水收费标志
                ...
                if (map.get("code".toUpperCase()).trim().equals("600000")) {// 返回成功有数据的情况下，才会插入所有的表
                    ...
                    if (type == 0) {
                        faceRecog.insertS(map);
                    } else {
                        faceRecog.updateS(map);// 数据库入表实时表更新
                    }
                } else {
                    // 接口调用异常
            ...
            List<Map<String, String>> listM = baseService.selectByStr("orm.mapper.zz.br.FaceRecogMapper.selectByCode", marking_cod);
