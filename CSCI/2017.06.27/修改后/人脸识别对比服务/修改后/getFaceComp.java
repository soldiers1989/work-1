    ...
    public String getFaceComp(HttpServletRequest request) {
        ...
        try {
            ...
            Map<String, String> map_code = transDtlService.getMarkingCode(request, jsonObject, "FaceCompServ");
            ...
            if (type != 2) {
                portrait_result = httpsUtil.postMsg(postBRUrl + "brFaceCompServ", "paramData=" + jsonObject.toString());
                ...
                // 取flag
                flag = map.get("flag_FaceCompServ".toUpperCase());
                // 更新流水收费标志
                if (String.valueOf(map.get("FLAG_FACECOMPSERV")).equals("1")) {
                    map.put("IS_FEE", "1");
                    map.put("TID", raid);
                } else {
                    map.put("IS_FEE", "0");
                ...
                if (map.get("code".toUpperCase()).trim().equals("600000")) {// 返回成功有数据的情况下，才会插入所有的表
                    ...
                    if (type == 0) {
                        faceComp.insertS(map);
                    } else {
                        faceComp.updateS(map);// 数据库入表实时表更新
                    }
                } else {
                    // 接口调用异常
            ...
            List<Map<String, String>> listM = baseService.selectByStr("orm.mapper.zz.br.FaceCompServMapper.selectByCode", marking_cod);
