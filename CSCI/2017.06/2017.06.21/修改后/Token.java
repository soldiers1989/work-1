...
public class Token {
    ...
    private static String WX_KEN_PopulationTag = "api.kyc.populationTag";// 人口属性标签查询服务
    /**
     * 获取微信配置
     * 
     * @param key
     ...
     * @return
     */
    public static String getpopulationTagFromKYC(String id, String type) {
        // 获取token
        String token = getAccessTokenFromCache();
        // 获取职业资格证书查询接口连接
        String icpcurl = Token.getkycProperty(Token.WX_KEN_PopulationTag);
        // 参数替换
        icpcurl = icpcurl.replace("{id}", id);
        icpcurl = icpcurl.replace("{type}", type);
        String access_token_str = null;
        try {
            ...
            logger.error("Token-->getpopulationTagFromKYC----" + e);
        }
        return access_token_str;
    }
}
