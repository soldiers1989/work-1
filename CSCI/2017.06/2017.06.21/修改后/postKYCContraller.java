...
/**
    ...
    /**
     * 
    * @Title: kycpopulationTag
    * @Description: 人口属性标签查询服务
    * @author ly
    * @date  2017年6月21日 上午11:51:04
    * @param request
    * @return
     */
    
    @RequestMapping(value = "/kycPopulationTag", produces = "text/html;charset=UTF-8")
    @ResponseBody
    public String kycpopulationTag(HttpServletRequest request) {
        String retvalue = null;
        try {
            ...
            String type = obj.getString("type");
            String id = obj.getString("id");
            retvalue = Token.getpopulationTagFromKYC(id, type);
        if (retvalue == null)
            logger.error("人口属性标签查询服务 返回为空!");
    } catch (Exception e) {
        logger.error("人口属性标签查询服务异常-------->>>" + e);
    }
    return retvalue;
    }
}
