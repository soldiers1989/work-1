...
public class LTbizController {
    ...
    @Autowired
    private GxminWorkService gxminWorkService;// 工作地验证
    ...
    public String GxminWork(HttpServletRequest request) {
        ...
        try {
            ...
            portrait_result = httpsUtil.postMsg(postLTUrl + "gxminWork", "paramData=" + jsonObjectTr.toString());
            // -----------json解析修改start------------
    ...
    public String UserWhite(HttpServletRequest request) {
        ...
        try {
            ...
            // 去后置请求数据
            JSONObject jsonObjectTr = jsonObject.discard("merchantId");
            portrait_result = httpsUtil.postMsg(postLTUrl + "userWhite", "paramData=" + jsonObjectTr.toString());
            // -----------json解析修改start------------
            map = (HashMap<String, String>) JsonUtil.getJsonMap(portrait_result, map, null);
            // 如果查询无数据或者查询失败则不入库
            if (!map.get("CODE").equals("600000")) {
            ...
            logger.info("getUserWhiteFaith_parmData 输出--->:" + portrait_result);
            // -----------json解析修改end------------
            UserWhiteService.insert(map);
            // 更新流水收费标志
            if ("".equals(portrait_result) || null == portrait_result) {
                logger.error("无返回 ");
            } else {
                logger.info(portrait_result + ":" + portrait_result);
                String[] keys1 = { "flag", "flag_userwhite" };
                flagmap = (HashMap<String, String>) JsonUtil.getJsonObject(portrait_result, keys1, 2, flagmap);
                if (flagmap.get("FLAG_USERWHITE").equals("1")) {
                    flagmap.put("IS_FEE", "1");
                    flagmap.put("TID", raid);
                } else {
                    flagmap.put("IS_FEE", "0");
            ...
            logger.error("getUserWhiteFaith_ERROR 输出--->:" + e);
        }
        return portrait_result;
    }
