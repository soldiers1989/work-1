...
public class BRbizController {
    ...
    public String EduLevel(HttpServletRequest request) {
        ...
        try {
            ...
            Map<String, String> map_code = transDtlService.getMarkingCode(request, jsonObjectTr, "EduLevel");
            ...
            if (type != 2) {
                portrait_result = httpsUtil.postMsg(postBRUrl + "EduLevel", "paramData=" + jsonObjectTr.toString());
                ...
                if (map.get("code".toUpperCase()).equals("600000")) {// 返回成功有数据的情况下，才会插入所有的表
                    ...
                    EduLevel.insert(map);
                    if (type == 0) {
                        EduLevel.insert_S(map);
                    } else {
                        EduLevel.update_S(map);// 数据库入表实时表更新
                    }
                } else {
                    // 接口调用异常
            ...
            List<Map<String, String>> listM = baseService.selectByStr("orm.mapper.zz.br.EduLevelMapper.selectByCode", marking_cod);
            ...
            transDtlService.updateTransDtl_N(sid, String.valueOf(map.get("flag_edulevel".toUpperCase())), type);// 更新流水收费标志
        } catch (Exception e) {
            logger.error("学历信息查询 :" + e);
        }
        return portrait_result + backTime;
    }
        ...
        try {
            ...
            if (type != 2) {
                ...
                if (map.get("code").equals("00")) {// 返回成功有数据的情况下，才会插入所有的表
                    ...
                    if (type == 0) {
                        listcServcie.Insert_SpecialList_S(map);// 实时表
                    } else {
    ...
    public String Execution(HttpServletRequest request) {
        ...
        try {
            ...
            // -----------从数据库中获取数据进行json字符串拼接start------------
            // 根据markingCode找到最新的一条主表数据
            Map<String, List<Map<String, String>>> mapM = new HashMap<>();
            List<Map<String, String>> listM = baseService.selectByStr("orm.mapper.zz.br.ExecutionMapper.selectByCode", marking_cod);
            mapM.put("result", listM);
            retvalue = JsonUtil.getJsonStr(null, jsonObjectTr, mapM);
            // -----------从数据库中获取数据进行json字符串拼接end------------
    ...
    public String TelPeriodCMCCf(HttpServletRequest request, HttpServletResponse response) {
        ...
        portrait_result = telPeriodCMCCfService.skipTo(request, paramData, postBRUrl, raid);
    ...
    public String TelPeriodCUCCd(HttpServletRequest request, HttpServletResponse response) {
        ...
        portrait_result = telPeriodCUCCdService.skipTo(request, paramData, postBRUrl, raid);
     ...
     * @Description: 电信手机在网时长
     * @author cxl
     * @date 2017年6月26日 下午8:14:31
     * @param request
     * @param response
     * @return
     */
    ...
    public String TelPeriodCTCCy(HttpServletRequest request, HttpServletResponse response) {
        ...
        portrait_result = telPeriodCTCCyService.skipTo(request, paramData, postBRUrl, raid);
    ...
    public String TelStatusCMCCf(HttpServletRequest request, HttpServletResponse response) {
        ...
        portrait_result = telStatusCMCCfServcie.skipTo(request, paramData, postBRUrl, raid);
    ...
    public String Consumption_c(HttpServletRequest request) {
        logger.info("前置进入后置--->" + System.currentTimeMillis());
        String retvalue = null;
        String flag = "";
        String backTime = "0";
        Map<String, String> map = new HashMap<String, String>();
        ...
        try {
            ...
            // 去后置请求数据
            JSONObject jsonObjectTr = jsonObject.discard("merchantId");
            // id cell name 入IDENTI_MAPPING 表
            ...
            if (type != 2) {
                ...
                logger.info("商品消费评估查询返回值正在进行入库:" + retvalue);
                // -----------json解析修改start------------
                map = JsonFormts.getInstantiation().getmap(retvalue);
                map.put("TIME", Util.getCurrentDateTime());
                map.put("ID", raid);
    ...
    public String TelStatus(HttpServletRequest request) {
        ...
        try {
            ...
            JSONObject jsonObjectTr = jsonObject.discard("merchantId");
            // id cell name 入IDENTI_MAPPING 表；且生成对应需要的markingCode
            Map<String, String> map_code = transDtlService.getMarkingCode(request, jsonObjectTr, "TelStatus");
            String marking_cod = map_code.get("MARKING_CODE");
            int type = Integer.parseInt(map_code.get("type"));// 0：需重新请求带三方接口获取数据1：数据库有数据超时，需重新获取2：数据库有数据未超时，正常
            // 判断数据库是否有数据，如果有则直接拼接，如果没有或超时需从第三方获取插入数据库后再拼接
            if (type != 2) {
                ...
                if (map.get("code".toUpperCase()).equals("600000")) {// 返回成功有数据的情况下，才会插入所有的表
                    ...
                    // 判断数据库是有数据超时还是无数据，超时，实时表主表更新，历史表直接插入
                    // 主表插入
                    statusService.insert(map);
                    if (type == 0) {
                        statusService.insert_S(map);
                    } else {
                        statusService.update_S(map);// 数据库入表实时表更新
                    }
                } else {
                    // 接口调用异常
                    retvalue = "{\"code\":\"2005\" ,\"msg\":\"接口异常\"}";
            ...
            List<Map<String, String>> listM = baseService.selectByStr("orm.mapper.zz.br.TelStatusServiceMapper.selectByCode", marking_cod);
            if (null != listM && listM.size() > 0) {
                mapM = listM.get(0);
            }
            retvalue = JsonUtil.getJsonStr(mapM, jsonObjectTr, null);
            ...
            transDtlService.updateTransDtl_N(raid, String.valueOf(map.get("flag_telstatus".toUpperCase())), type);// 更新流水收费标志
        } catch (Exception e) {
            logger.error("手机在网状态查询:" + e);
        }
        return retvalue + backTime;
    }
    ...
    public String TelStateCUCC_d(HttpServletRequest request) {
        ...
        try {
            ...
            JSONObject jsonObjectTr = jsonObject.discard("merchantId");
            // id cell name 入IDENTI_MAPPING 表；且生成对应需要的markingCode
            Map<String, String> map_code = transDtlService.getMarkingCode(request, jsonObjectTr, "TelStateCUCC_d");
            String marking_cod = map_code.get("MARKING_CODE");
            int type = Integer.parseInt(map_code.get("type"));// 0：需重新请求带三方接口获取数据1：数据库有数据超时，需重新获取2：数据库有数据未超时，正常
            // 判断数据库是否有数据，如果有则直接拼接，如果没有或超时需从第三方获取插入数据库后再拼接
            if (type != 2) {
                ...
                if (map.get("code".toUpperCase()).equals("600000")) {// 返回成功有数据的情况下，才会插入所有的表
                    ...
                    if (type == 0) {
                        telStateCUCCdService.insert_S(map);
                    } else {
                        telStateCUCCdService.update_S(map);// 数据库入表实时表更新
                    }
                } else {
                    // 接口调用异常
                    portrait_result = "{\"code\":\"2005\" ,\"msg\":\"接口异常\"}";
            ...
            transDtlService.updateTransDtl_N(raid, String.valueOf(map.get("FLAG_TELSTATECUCC_D".toUpperCase())), type);// 更新流水收费标志
        } catch (Exception e) {
            logger.error("联通手机在网状态查询:" + e);
        }
        return portrait_result + backTime;
    }
    ...
    public String getIdTwo_z(HttpServletRequest request) {
        ...
        try {
            ...
            // id cell name 入IDENTI_MAPPING 表；且生成对应需要的markingCode
            Map<String, String> map_code = transDtlService.getMarkingCode(request, jsonObjectTr, "IdTwoserver");
            String marking_cod = map_code.get("MARKING_CODE");
            int type = Integer.parseInt(map_code.get("type"));// 0：需重新请求带三方接口获取数据1：数据库有数据超时，需重新获取2：数据库有数据未超时，正常
            // 判断数据库是否有数据，如果有则直接拼接，如果没有或超时需从第三方获取插入数据库后再拼接
            if (type != 2) {
            ...
            List<Map<String, String>> listM = baseService.selectByStr("orm.mapper.zz.br.idTwoMapper.selectByCode", marking_cod);
            if (null != listM && listM.size() > 0) {
                mapM = listM.get(0);
            }
            retvalue = JsonUtil.getJsonStr(mapM, jsonObjectTr, null);
            ...
            transDtlService.updateTransDtl_N(sid, String.valueOf(map.get("FLAG_IDTWO_Z")), type);// 更新流水收费标志
        } catch (Exception e) {
            logger.error("身份证二要素查询 :" + e);
        }
        return retvalue + backTime;
    }
    ...
    public String BankThree(HttpServletRequest request) {
        ...
        try {
            ...
            // id cell name 入IDENTI_MAPPING 表；且生成对应需要的markingCode
            Map<String, String> map_code = transDtlService.getMarkingCode(request, jsonObject, "BankThree");
            String marking_cod = map_code.get("MARKING_CODE");
            int type = Integer.parseInt(map_code.get("type"));// 0：需重新请求带三方接口获取数据1：数据库有数据超时，需重新获取2：数据库有数据未超时，正常
            // 判断数据库是否有数据，如果有则直接拼接，如果没有或超时需从第三方获取插入数据库后再拼接
            if (type != 2) {
                portrait_result = httpsUtil.postMsg(postBRUrl + "brBankThree", "paramData=" + jsonObject.toString());
                backTime = portrait_result.substring(portrait_result.lastIndexOf("}") + 1);
                // -----------json解析修改start------------
                map = JsonUtil.getJsonMap(portrait_result, map, null);
                // -----------json解析修改end------------
                ...
                // 取flag
                flag = map.get("flag_BankThree".toUpperCase());
                // 更新流水收费标志
                if (String.valueOf(map.get("FLAG_BANKTHREE")).equals("1")) {
                    map.put("IS_FEE", "1");
                    map.put("TID", raid);
                } else {
                    map.put("IS_FEE", "0");
                ...
                if (map.get("code".toUpperCase()).trim().equals("600000")) {// 返回成功有数据的情况下，才会插入所有的表
                    ...
                    if (type == 0) {
                        bankThreeService.insertS(map);
                    } else {
                        bankThreeService.updateS(map);// 数据库入表实时表更新
                    }
                } else {
                    // 接口调用异常
                    portrait_result = "{\"code\":\"2005\" ,\"msg\":\"接口异常\"}";
            ...
            // 根据markingCode找到最新的一条主表数据
            Map<String, String> mapM = new HashMap<>();
            List<Map<String, String>> listM = baseService.selectByStr("orm.mapper.zz.br.BankThreeMapper.selectByCode", marking_cod);
            if (null != listM && listM.size() > 0) {
                mapM = listM.get(0);
            }
            portrait_result = JsonUtil.getJsonStr(mapM, jsonObject, null);
