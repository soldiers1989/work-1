@RequestMapping(value = "/Undertaker", produces = "text/html;charset=UTF-8")
    ...
    public String Undertaker(HttpServletRequest request) {
        ...
        try {
            logger.info("server 处理  被执行人信息查询请求");
            ...
            // 记录客户请求流水
            int re = transDtlService.insertTransDtl("Undertaker", parmes, raid, "1", merId);
            if (re != 1) {
                logger.error("  被执行人信息查询，插入请求流水失败！ ");
            }
            // 记录请求数源流水
            int ret = transDtlService.insertTransDtl("ltUndertaker", parmes, raid, "2", merId);
            if (ret != 1) {
                logger.error("  被执行人信息查询，插入请求数源流水失败！ ");
            }
            // 去后置请求数据
            ...
            Map<String, String> map_code = transDtlService.getMarkingCode(request, jsonObjectTr, "Undertaker");
            ...
            if (type != 2) {
                ...
                retvalue = httpsUtil.postMsg(postLTUrl + "Undertaker", "paramData=" + jsonObjectTr.toString());
                backTime = retvalue.substring(retvalue.lastIndexOf("}") + 1);

                logger.info("  被执行人信息查询返回值正在进行入库");
                // -----------json解析修改start------------
                ...
                if (map.get("status".toUpperCase()).equals("1")) {// 返回成功，才会插入所有的表map.get("code".toUpperCase()).equals("600000")
                    ...
                    // 主表插入
                    undertakerService.Insert_Undertaker(map);
                    if (type == 0) {
                        undertakerService.Insert_Undertaker_S(map);
                    } else {
                        undertakerService.update_S(map);// 数据库入表实时表更新
                    }
                } else {
                    flag = "0";
                    // 接口调用异常
            ...
            List<Map<String, String>> listM = baseService.selectByStr("orm.mapper.zz.lt.UndertakerMapper.selectByCode", marking_cod);
