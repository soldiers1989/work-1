		...
		for (int i = 0; i < distinctTradeDate.size(); i++) {
			...
			for (int j = 0; j < catiList.size(); j++) {
				...
				for (int k = 0; k < ruleCodes.size(); k++) {
					...
					// 获取该客户,在该分析规则代码下的信息
					clientInfos = myBatisSessionTemplate.selectList("com.aif.rpt.biz.aml.reporting.server.reportingServerSql.selectClientInfos", parameter);
					if (clientInfos == null || clientInfos.size() <= 0) {
						continue;
					}
					List<Map> tradeInfoList;
					int serialNum3 = 1;
					for (int m = 0; m < clientInfos.size(); m++) {
						// serialNum3 = m + 1;
						clientInfoMap  = clientInfos.get(m);
						parameter.put("CLIENT_NAME", clientInfoMap.get("CLIENT_NAME"));
						parameter.put("CLIENT_IC_TYPE", clientInfoMap.get("CLIENT_IC_TYPE"));
						parameter.put("CLIENT_IC_NO", clientInfoMap.get("CLIENT_IC_NO"));
						parameter.put("CTPY_IC_OTHER_TYPE", clientInfoMap.get("CTPY_IC_OTHER_TYPE"));
						...
						int serialNum4 = 1;
						String refNo;
						for (int n = 0; n < tradeInfoList.size(); n++) {
						    // serialNum3 = m + 1;
						    ...
						    parameter.put("SERIAL_NUM_4", serialNum4);
						    ...
						    myBatisSessionTemplate.update("com.aif.rpt.biz.aml.reporting.server.reportingServerSql.updateSerialNum4", parameter);
						    serialNum4++;
						}
						serialNum3++;
					}
