USE [SOAR]
GO
/****** Object:  StoredProcedure [dbo].[SP_HUGETRANS_0503]    Script Date: 2017/9/5 15:27:20 ******/
SET ANSI_NULLS ON
GO
...
************************************************************************************************/
ALTER PROCEDURE [dbo].[SP_HUGETRANS_0503]
  @DEPARTID       VARCHAR(20),
  @WORKDATE       VARCHAR(8),
...
BEGIN
  ...
  --参数定义
  DECLARE @RULE_CODE     VARCHAR(15)  = '0503'    --规则代码   0503
...
BEGIN TRY  
  ...
  DECLARE CURSOR_0503 CURSOR FOR
  ...
  FETCH NEXT FROM CURSOR_0503 INTO @CLIENT_ID
...
BEGIN CATCH
    ...
    IF (@ERR_CODE != 0)
    BEGIN     
        SET @ERR_MSG = 'SP_HUGETRANS_0503:' + ERROR_MESSAGE ()
        RETURN
    END 
END  CATCH
