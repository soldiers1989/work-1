USE [SOAR]
GO
/****** Object:  StoredProcedure [dbo].[SP_HUGETRANS_0902]    Script Date: 2017/9/4 18:07:51 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
  ...
  --参数定义
  DECLARE @RULE_CODE     VARCHAR(15)              --规则代码   0902
...
BEGIN TRY  
  ...
  --建立分析统计游标      
  DECLARE CURSOR_0902_CNY CURSOR FOR
  ...
  --打开游标
  OPEN CURSOR_0902_CNY 
  
  --初始化参数
  SET @CLIENT_ID = ''
  SET @DEBIT_CREDIT = ''
  
  FETCH NEXT FROM CURSOR_0902_CNY INTO @CLIENT_ID,@DEBIT_CREDIT
  ...
  PRINT  @WHERE_SQL
    ...
    BEGIN      
      --关闭游标
      CLOSE CURSOR_0902_CNY
      DEALLOCATE CURSOR_0902_CNY
  ...
  ******************************************************************/  
  --建立分析统计游标      
  DECLARE CURSOR_0902_USD CURSOR FOR
    SELECT CLIENT_ID, DEBIT_CREDIT
        FROM AML_DATASOURCE
    WHERE WORK_DATE = @WORKDATE
        AND DEPART_ID = @DEPARTID
  ...
  --打开游标
  OPEN CURSOR_0902_USD 
  
  --初始化参数
  SET @CLIENT_ID = ''
  SET @DEBIT_CREDIT = ''
  
  FETCH NEXT FROM CURSOR_0902_USD INTO @CLIENT_ID,@DEBIT_CREDIT
  ...
  PRINT  @WHERE_SQL
    ...
    BEGIN      
      --关闭游标
      CLOSE CURSOR_0902_USD
      DEALLOCATE CURSOR_0902_USD
